...
import com.hytowny.HyTownyAccess;
...
public class ClaimImageBuilder {
    ...
    private ClaimImageBuilder generateImageAsync() {
        ...
        UUID claimOwner = HyTownyAccess.getClaimOwner(worldName, chunkX, chunkZ);
        ...
        UUID[] nearbyOwners = new UUID[]{
                HyTownyAccess.getClaimOwner(this.worldChunk.getWorld().getName(), chunkX, chunkZ + 1), // SOUTH
                HyTownyAccess.getClaimOwner(this.worldChunk.getWorld().getName(), chunkX, chunkZ - 1), // NORTH
                HyTownyAccess.getClaimOwner(this.worldChunk.getWorld().getName(), chunkX + 1, chunkZ), // EAST
                HyTownyAccess.getClaimOwner(this.worldChunk.getWorld().getName(), chunkX - 1, chunkZ), // WEST
        };
    ...
    private void drawClaimText(String worldName, int chunkX, int chunkZ) {
        String ownerName = HyTownyAccess.getOwnerName(worldName, chunkX, chunkZ);
        List<String> trustedNames = HyTownyAccess.getTrustedPlayerNames(worldName, chunkX, chunkZ);
