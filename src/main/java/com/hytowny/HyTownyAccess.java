...
public class HyTownyAccess {
    ...
    public static void init(ClaimStorage storage) {
        ...
        System.out.println("[HyTownyAccess] Initialized with claimStorage: " + (storage != null ? "OK" : "NULL"));
    }
    ...
    public static UUID getClaimOwner(String worldName, int chunkX, int chunkZ) {
        if (claimStorage == null) {
            System.out.println("[HyTownyAccess] ERROR: claimStorage is null!");
            return null;
        }
        UUID owner = claimStorage.getClaimOwner(worldName, chunkX, chunkZ);
        // Debug: log when we find a claim (first few only to avoid spam)
        if (owner != null) {
            System.out.println("[HyTownyAccess] Found claim at " + worldName + " " + chunkX + "," + chunkZ + " owner=" + owner);
        }
        return owner;
    }
