...
import com.hytowny.gui.ChunkVisualizerGui;
import com.hytowny.gui.ClaimSettingsGui;
import com.hytowny.config.PluginConfig;
import com.hytowny.data.Claim;
import com.hytowny.data.PlayerClaims;
import com.hytowny.data.TrustLevel;
import com.hytowny.data.TrustedPlayer;
...
/**
 * Main command for HyTowny plugin.
...
public class ClaimCommand extends AbstractPlayerCommand {
    ...
    private void showAdminHelp(PlayerRef playerData) {
        playerData.sendMessage(Message.raw("=== HyTowny Admin Commands ===").color(GOLD));
    ...
    private void handleUnclaimAll(PlayerRef playerData) {
        ...
        if (count > 0) {
            ...
            for (String worldName : HyTowny.WORLDS.keySet()) {
    ...
    private void showHelp(PlayerRef playerData) {
        playerData.sendMessage(Message.raw("=== HyTowny Commands ===").color(GOLD));
    ...
    private boolean hasAdminPermission(PlayerRef playerData) {
        if (!PermissionsModule.get().hasPermission(playerData.getUuid(), "hytowny.admin")) {
