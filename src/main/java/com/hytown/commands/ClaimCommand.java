...
import com.hytown.gui.ChunkVisualizerGui;
import com.hytown.gui.ClaimSettingsGui;
import com.hytown.config.PluginConfig;
import com.hytown.data.Claim;
...
/**
 * Main command for HyTown plugin.
...
public class ClaimCommand extends AbstractPlayerCommand {
    ...
    private void showAdminHelp(PlayerRef playerData) {
        playerData.sendMessage(Message.raw("=== HyTown Admin Commands ===").color(GOLD));
    ...
    private void handleUnclaimAll(PlayerRef playerData) {
        ...
        if (count > 0) {
            ...
            for (String worldName : HyTown.WORLDS.keySet()) {
    ...
    private boolean hasAdminPermission(PlayerRef playerData) {
        if (!PermissionsModule.get().hasPermission(playerData.getUuid(), "hytown.admin")) {
